...
public final class Program1PrettyPrint1 extends Program1 {
    ...
    public void prettyPrint(SimpleWriter out) {
        ...
        String tmp = "";
        String name = this.replaceName(tmp);
        tmp = this.replaceName(name);
        ...
        while (ctxt.size() > 0) {
            ...
            String instrName = instr.key();
            out.print("INSTRUCTION " + instrName + " IS");
            out.println();
            Statement s = instr.value();
            s.prettyPrint(out, INDENT_SIZE + 4);
            printSpaces(out, INDENT_SIZE);
            out.println("END " + instrName);
            c.add(instrName, s);
        }
        this.replaceContext(c);
        Statement b = this.newBody();
        Statement btxt = this.replaceBody(b);
        out.println();
        out.println("BEGIN");
        btxt.prettyPrint(out, INDENT_SIZE);
        out.println("END " + name);
        this.replaceBody(btxt);
    }
