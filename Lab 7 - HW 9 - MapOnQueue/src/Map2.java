...
public class Map2<K, V> extends MapSecondary<K, V> {
    ...
    private static <K, V> void moveToFront(Queue<Pair<K, V>> q, K key) {
        ...
        int l = 0;
        while (l < q.length() && !q.front().key().equals(key)) {
            Pair<K, V> remove = q.dequeue();
            q.enqueue(remove);
            l++;
        }
    }
    ...
    public final void add(K key, V value) {
        ...
        Pair<K, V> element = new SimplePair<>(key, value);
        this.pairsQueue.enqueue(element);
    }
    ...
    public final V value(K key) {
        assert key != null : "Violation of: key is not null";
        assert this.hasKey(key) : "Violation of: key is in DOMAIN(this)";
        assert this.pairsQueue
                .length() > 0 : "Violation of: this.pairsQueue.length /= 0";
        moveToFront(this.pairsQueue, key);
    ...
    public final boolean hasKey(K key) {
        ...
        if (this.pairsQueue.length() != 0) {
            Pair<K, V> check = this.pairsQueue.dequeue();
            this.pairsQueue.enqueue(check);
            if (check.key() == key) {
                result = true;
            }
        }
