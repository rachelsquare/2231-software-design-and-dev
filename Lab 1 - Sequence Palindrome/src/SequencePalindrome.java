...
public final class SequencePalindrome {
    ...
    private static boolean isPalindrome(Sequence<Integer> s) {
        ...
        Sequence<Integer> tmp = new Sequence1L<>();
        Stack<Integer> back = new Stack1L<>();
        boolean result = true;
        while (s.length() > 1) {
            int f = s.remove(0);
            tmp.add(tmp.length(), f);
            int l = s.remove(s.length() - 1);
            back.push(l);
            if (f != l) {
                result = false;
                break;
        ...
        while (s.length() != 0) {
            int m = s.remove(0);
            tmp.add(tmp.length(), m);
        }
        Iterator<Integer> it = back.iterator();
        while (it.hasNext()) {
            tmp.add(tmp.length(), it.next());
        }
        s.transferFrom(tmp);
        return result;
    }
