...
public final class Statement1PrettyPrint1 extends Statement1 {
    ...
    public void prettyPrint(SimpleWriter out, int offset) {
        switch (this.kind()) {
            case BLOCK: {
                for (int l = 0; l < this.lengthOfBlock(); l++) {
                    Statement ns = this.removeFromBlock(l);
                    ns.prettyPrint(out, offset + 4);
                    this.addToBlock(l, ns);
                }
                break;
            }
            case IF: {
            ...
            case CALL: {
                String str = this.disassembleCall();
