 ...
 *
 */
public final class renameProgram {

    /**
     * Private constructor so this utility class cannot be instantiated.
     */
    private renameProgram() {
    }
    ...
    public static void renameInstruction(Program p, String oldName,
        ...
        while (ctxt.size() > 0) {
            ...
            if (instrName.equals(oldName)) {
                instrName = newName;
            }
            Statement s = instr.value();
            renameInstruction(s, oldName, newName);
            c.add(instrName, s);
        }
        p.replaceContext(c);
        Statement b = p.newBody();
        Statement btxt = p.replaceBody(b);
        renameInstruction(btxt, oldName, newName);
        p.replaceBody(btxt);
