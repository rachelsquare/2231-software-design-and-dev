...
public class Set2<T> extends SetSecondary<T> {
    ...
    private static <T> void moveToFront(Queue<T> q, T x) {
        ...
        // use && because when length=0 method will still call
        // q.front and will give indexOutOfBoundError
        // demorgan's law either length==0 || front==x
        while (q.length() != 0 && q.front() != x) {
            T remove = q.dequeue();
            tmp.enqueue(remove);
        }
        while (tmp.length() != 0) {
            T transfer = tmp.dequeue();
            q.enqueue(transfer);
        }
    }
